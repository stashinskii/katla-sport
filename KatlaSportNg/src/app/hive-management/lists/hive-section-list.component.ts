...
export class HiveSectionListComponent implements OnInit {
  ...
  constructor(
    ...
    private hiveSectionsService: HiveSectionService
  ) { }
  ...
  onDelete(hiveSectionId: number){
    ...
    this.hiveSectionsService.setHiveSectionStatus(hiveSectionId, true).subscribe(c => hive.isDeleted = true);
  }

  onUndelete(hiveSectionId: number){
    var hive = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionsService.setHiveSectionStatus(hiveSectionId, false).subscribe(c => hive.isDeleted = false);
  }
}
