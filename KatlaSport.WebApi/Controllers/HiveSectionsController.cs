...
namespace KatlaSport.WebApi.Controllers
{
    ...
    public class HiveSectionsController : ApiController
    {
        ...
        [SwaggerResponse(HttpStatusCode.InternalServerError)]
        public async Task<IHttpActionResult> AddHive([FromBody] UpdateHiveSectionRequest hiveRequest)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            var hiveSection = await _hiveSectionService.CreateHiveSectionAsync(hiveRequest);
